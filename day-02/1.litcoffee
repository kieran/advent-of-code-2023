...
## Defs
    ...
    # filters out impossible games, returns game ids
    possibleIds = (games=[])->
      games
      .filter (game)->
        for {colour, num} in game.plays
          return false if num > 12 and colour is 'red'
          return false if num > 13 and colour is 'green'
          return false if num > 14 and colour is 'blue'
        true
      .map ({id})-> id
...
## Tests
    ...
    assert.equal 8, sum ...possibleIds games inputLines input


## Run

    log sum ...possibleIds games inputLines getInput()
